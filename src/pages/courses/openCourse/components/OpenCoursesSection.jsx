  ...
  return (
    <section className="bg-white mb-100">
      <div className="p-10 flex justify-between items-center mobil2:flex-col mobil2:items-start">
        ...
        <Link to={props.link}>
          <p className="font-400 text-text14 py-14 px-16 border border-shadow rounded-6  text-nowrap hover:bg-primary hover:text-white">
            {" "}
            View Course
          </p>
        </Link>
      ...
      <div className="images">
        ...
        <p className="px-24 py-20 font-600 text-18 border-t border-l border-r m-2 border-shadow rounded-8">
          Curriculum
        </p>
        <div className="grid grid-cols-6 gap-0 p-20 w-full mx-auto border rounded-6 border-shadow  table1080:grid table1080:grid-cols-[repeat(auto-fill,minmax(300px,1fr))] table1080:gap-18  ">
          {coursesData.map((item, index) => (
            <div
              key={index}
              className="number w-full  rounded-8 py-10 px-30 border-r border-shadow  table1080:bg-shadow table1080:px-20 "
            >
